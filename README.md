# Murotal Audio Editor 0.4.0

Offline Android audio editor. Build with JDK 17, Gradle 8.9, Android SDK 35; minimum Android 8.

## Playback
- Select Semua track and press play to preview all unmuted tracks from the gold playhead.
- Select Track terpilih and press play to preview all clips on the selected clip's track.
- The pause button pauses at the current position; press play to resume.
...
- Ekspor mixes all tracks into WAV (16-bit stereo 44.1 kHz), M4A/AAC or MP3 (128/192/320 kbps).
...
- FLAC export is not implemented.
...
- Tap/drag the ruler to set the playhead, then Split; select the segment to edit/delete.
...
## Tests
GitHub Actions runs unit tests and builds a debug APK. Android 10 emulator tests render two overlapping generated tones, verify mixed amplitude and duration, round-trip a project, produce AAC output, exercise trim/speed/pitch, and check playback completion/mode switching. They also decode each MP3 quality, check audible output and file-size ordering, exercise compact menus on a small screen, and record an export timing comparison against 0.3.0.

## Still pending
Waveform, fade, undo/redo, bundled portable projects, FLAC encoder and automatic edge scrolling.
